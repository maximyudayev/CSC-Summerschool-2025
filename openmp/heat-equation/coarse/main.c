...
int main(int argc, char **argv) {
  ...
  #pragma omp parallel
  {
    ...
    for (int iter = 1; iter <= nsteps; iter++) {
      //#pragma omp barrier
      // Launch non-blocking exchange
      #pragma omp single
      exchange_immediate(&previous, &parallelization, requests);

      // Calculate local temperature field away from border
      evolve_local(&current, &previous, a, dt);

      // Finalize non-blocking exchange
      #pragma omp single
      exchange_wait(&previous, &parallelization, requests);

      // Calculate local temperature field at the border after exchange
      evolve_boundary(&current, &previous, a, dt);
